...
// Canvas-drawn sliders for iframe compatibility
...
function setup() {
    ...
    zetaSlider = new CanvasSlider(140, canvasHeight - 55, 220, 0.05, 0.95, 0.2, '\u03B6 (damping ratio)');
    omegaSlider = new CanvasSlider(140, canvasHeight - 25, 220, 1, 20, 5, '\u03C9\u2080 (natural freq)');
    sliders = [zetaSlider, omegaSlider];
}
...
function draw() {
    ...
    let phi = acos(zeta);
    let overshootPct = 100 * exp(-PI * zeta / sqrtTerm);
    ...
    // Draw grid and axes
    drawGrid(tMax, vMin, vMax);
    drawAxes(tMax, vMin, vMax);
...
function drawGrid(tMax, vMin, vMax) {
    stroke(gridColor);
    strokeWeight(0.5);
...
function drawAxes(tMax, vMin, vMax) {
    stroke(axisColor);
    strokeWeight(1.5);
...
function mousePressed() {
    for (let s of sliders) s.pressed(mouseX, mouseY);
}

function mouseDragged() {
    for (let s of sliders) s.dragged(mouseX);
}

function mouseReleased() {
    for (let s of sliders) s.released();
}
